...
public class AdminInitRoleManagementEventTestExpectedEventPayloadBuilder {
    ...
    /**
     * Builds the expected payload for a role permissions updated event.
     */
    public static JSONObject buildExpectedRolePermissionsUpdatedEventPayload(String tenantDomain, String appId,
        ...
        role.put("removedPermissions", new JSONArray().put(removedPermission));
        event.put("role", role);
    ...
    public static JSONObject buildExpectedRoleUsersAddedEventPayload(String tenantDomain, String appId, String appName,
        ...
        JSONObject role = buildBaseRole(roleName, appId, appName);
        role.put("addedUsers", new JSONArray().put(createRoleUser(userId, username, tenantDomain)));
        event.put("role", role);
    ...
    /**
     * Builds the expected payload for a role users updated event where a regular user and an agent are added while a
     * regular user is removed.
     */
    public static JSONObject buildExpectedRoleUsersUpdatedEventPayload(String tenantDomain, String appId,
                                                                       String appName, String roleName,
                                                                       String addedUserId, String addedUsername,
                                                                       String agentId, String agentName,
                                                                       String removedUserId, String removedUsername)
            throws Exception {
        ...
        role.put("addedUsers", new JSONArray()
                .put(createRoleUser(addedUserId, addedUsername, tenantDomain))
                .put(createRoleAgent(agentId, agentName, tenantDomain)));
        role.put("removedUsers", new JSONArray()
                .put(createRoleUser(removedUserId, removedUsername, tenantDomain)));
        event.put("role", role);
    ...
    /**
     * Builds the expected payload for a role deleted event. The delete event does not carry an initiatorType and the
     * role object contains only its id.
     */
    public static JSONObject buildExpectedRoleDeletedEventPayload(String tenantDomain) throws Exception {
        ...
        event.put("action", ACTION_ROLE_DELETE);
    ...
    private static JSONObject createRoleUser(String userId, String username, String tenantDomain) throws JSONException {
        ...
        user.put("ref", buildUserRef(userId, tenantDomain));
        return user;
    }

    private static JSONObject createRoleAgent(String agentId, String agentName, String tenantDomain)
        ...
        agent.put("ref", buildUserRef(agentId, tenantDomain));
        return agent;
    }
    ...
    private static String buildUserRef(String userId, String tenantDomain) {

        if (tenantDomain.equals(MultitenantConstants.SUPER_TENANT_DOMAIN_NAME)) {
            return SERVER_BASE_URL + "/scim2/Users/" + userId;
        }
        return SERVER_BASE_URL + "/t/" + tenantDomain + "/scim2/Users/" + userId;
    }
}
